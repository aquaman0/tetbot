...
def bestdeal(data: dict) -> Iterable[tuple]:
    """
    Функция, которая осуществляет поиск отелей, фильтруя их по удаленности от центра в заданном
    ценовом диапазоне, получая данные от пользователя.
